...
const BOARD_TOKEN = "databricks";
const OFFICIAL_BOARD_URL = "https://www.databricks.com/company/careers/open-positions";
const GREENHOUSE_BOARD_API = `https://boards-api.greenhouse.io/v1/boards/${BOARD_TOKEN}/jobs`;
const GREENHOUSE_JOB_BOARD_URL = `https://boards.greenhouse.io/${BOARD_TOKEN}`;
const USER_AGENT = "EarlyApply generated Databricks scraper";
...
function officialJobUrl(jobId: string): string {
	const url = new URL(OFFICIAL_BOARD_URL);
	url.searchParams.set("gh_jid", jobId);
	return url.toString();
}
...
async function fetchGreenhouseJobs(ctx: ScraperCompanyContext): Promise<GreenhouseJob[]> {
	...
	await ctx.logger.info("Fetching Databricks Greenhouse jobs", {
		url: url.toString(),
	...
	if (!response.ok) {
		throw new Error(`Databricks Greenhouse jobs request failed with HTTP ${response.status}`);
	}
...
async function fetchGreenhouseJob(jobId: string): Promise<FetchJobResult> {
	...
	if (response.status === 404 || response.status === 410) {
		return {
			status: "removed",
			message: `Databricks Greenhouse job ${jobId} is no longer available`,
		};
	}
	if (!response.ok) {
		if (response.status >= 400 && response.status < 500) {
			return {
				status: "removed",
				message: `Databricks Greenhouse job ${jobId} returned HTTP ${response.status}`,
			};
		}
		throw new Error(`Databricks Greenhouse job ${jobId} request failed with HTTP ${response.status}`);
	}
	...
	if (!compactText(job.title)) {
		return {
			status: "removed",
			message: `Databricks Greenhouse job ${jobId} did not return an active posting`,
		};
	}
...
function extractSponsorship(job: GreenhouseJob, description: string | null): {
	...
	const sponsorshipText = questionText ?? descriptionText ?? null;
...
export async function firstRun(ctx: ScraperCompanyContext): Promise<FirstRunResult> {
	...
	await ctx.logger.info("Fetched Databricks jobs", {
...
export async function secondRun(ctx: SecondRunContext): Promise<SecondRunResult> {
	...
	if (!jobId) {
		return {
			status: "removed",
			message: "Databricks job URL no longer contains a Greenhouse job id",
			raw: {
				jobUrl: ctx.jobUrl,
				source: "greenhouse-board-api",
			},
	...
	await ctx.logger.info("Fetching Databricks job detail", {
		jobId,
		jobUrl: ctx.jobUrl,
	});
	...
	const locations = normalizeLocations(job);
	const workplaceType = metadataValue(job, /workplace/i) ?? inferWorkplaceType(locations, description);
	const salary = parseSalary(description);
	const sponsorship = extractSponsorship(job, description);
	const title = compactText(job.title) ?? "Databricks role";
	...
	return {
		...
		raw: {
			...
			boardToken: BOARD_TOKEN,
			jobUrl: ctx.jobUrl,
			greenhouseUrl: normalizeUrl(job.absolute_url) ?? `${GREENHOUSE_JOB_BOARD_URL}/jobs/${jobId}`,
		},
	};
}
