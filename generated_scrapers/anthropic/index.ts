...
const OFFICIAL_BOARD_URL = "https://www.anthropic.com/jobs";
const GREENHOUSE_BOARD_API = `https://boards-api.greenhouse.io/v1/boards/${BOARD_TOKEN}/jobs`;
const GREENHOUSE_JOB_BOARD_URL = `https://job-boards.greenhouse.io/${BOARD_TOKEN}`;
const USER_AGENT = "EarlyApply generated Anthropic scraper";
...
function canonicalJobUrl(job: GreenhouseJob): string {
	...
	return id ? `${GREENHOUSE_JOB_BOARD_URL}/jobs/${encodeURIComponent(id)}` : OFFICIAL_BOARD_URL;
}
...
async function fetchGreenhouseJobs(ctx: ScraperCompanyContext): Promise<GreenhouseJob[]> {
	...
	await ctx.logger.info("Fetching Anthropic Greenhouse jobs", {
		url: url.toString(),
	...
	if (!response.ok) {
		throw new Error(`Anthropic Greenhouse jobs request failed with HTTP ${response.status}`);
	}
...
async function fetchGreenhouseJob(jobId: string): Promise<FetchJobResult> {
	...
	if (response.status === 404 || response.status === 410) {
		return {
			status: "removed",
			message: `Anthropic Greenhouse job ${jobId} is no longer available`,
		};
	}
	if (!response.ok) {
		if (response.status >= 400 && response.status < 500) {
			return {
				status: "removed",
				message: `Anthropic Greenhouse job ${jobId} returned HTTP ${response.status}`,
			};
		}
		throw new Error(`Anthropic Greenhouse job ${jobId} request failed with HTTP ${response.status}`);
	}
	...
	if (!compactText(job.title)) {
		return {
			status: "removed",
			message: `Anthropic Greenhouse job ${jobId} did not return an active posting`,
		};
	}
...
function normalizeLocations(job: GreenhouseJob): string[] {
	const values = [
		...
		...(job.offices ?? []).flatMap((office) => [
			compactText(office.name),
			compactText(office.location),
		]),
...
export async function firstRun(ctx: ScraperCompanyContext): Promise<FirstRunResult> {
	...
	await ctx.logger.info("Fetched Anthropic jobs", {
...
export async function secondRun(ctx: SecondRunContext): Promise<SecondRunResult> {
	...
	if (!jobId) {
		return {
			status: "removed",
			message: "Anthropic job URL no longer contains a Greenhouse job id",
			raw: {
				jobUrl: ctx.jobUrl,
				source: "greenhouse-board-api",
			},
	...
	await ctx.logger.info("Fetching Anthropic job detail", {
		jobId,
		jobUrl: ctx.jobUrl,
	});
	...
	const title = compactText(job.title) ?? "Anthropic role";
